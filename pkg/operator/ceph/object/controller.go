...
var ObjectStoreResourceCephLegacy = opkit.CustomResource{
	Name:    customResourceName,
	Plural:  customResourceNamePlural,
...
func (c *ObjectStoreController) watchLegacyObjectStores(namespace string, stopCh chan struct{}, resourceHandlerFuncs cache.ResourceEventHandlerFuncs) {
	...
	// watch for objectstore.ceph.rook.io/v1alpha1 events if the CRD exists
	if _, err := c.context.RookClientset.CephV1alpha1().ObjectStores(namespace).List(metav1.ListOptions{}); err != nil {
		logger.Infof("skipping watching for legacy ceph objectstore events (legacy objectstore CRD probably doesn't exist): %+v", err)
	} else {
		logger.Infof("start watching legacy ceph objectstores in all namespaces")
		watcherLegacy := opkit.NewWatcher(ObjectStoreResourceCephLegacy, namespace, resourceHandlerFuncs, c.context.RookClientset.CephV1alpha1().RESTClient())
...
func getObjectStoreObject(obj interface{}) (objectstore *cephv1beta1.ObjectStore, migrationNeeded bool, err error) {
	...
	objectStoreCephLegacy, ok := obj.(*cephv1alpha1.ObjectStore)
	if ok {
		return convertCephLegacyObjectStore(objectStoreCephLegacy.DeepCopy()), true, nil
	}
...
func (c *ObjectStoreController) migrateObjectStoreObject(objectstoreToMigrate *cephv1beta1.ObjectStore, legacyObj interface{}) error {
	...
	} else if _, ok := legacyObj.(*cephv1alpha1.ObjectStore); ok {
		logger.Infof("deleting legacy ceph objectstore %s in namespace %s", objectstoreToMigrate.Name, objectstoreToMigrate.Namespace)
		return c.context.RookClientset.CephV1alpha1().ObjectStores(objectstoreToMigrate.Namespace).Delete(
			objectstoreToMigrate.Name, &metav1.DeleteOptions{PropagationPolicy: &deletePropagation})
	}
...
func convertCephLegacyObjectStore(legacyObjectStore *cephv1alpha1.ObjectStore) *cephv1beta1.ObjectStore {
	if legacyObjectStore == nil {
		return nil
	}
	...
	objectStore := &cephv1beta1.ObjectStore{
		...
		Spec: cephv1beta1.ObjectStoreSpec{
			MetadataPool: pool.ConvertCephLegacyPoolSpec(legacySpec.MetadataPool),
			DataPool:     pool.ConvertCephLegacyPoolSpec(legacySpec.DataPool),
			Gateway: cephv1beta1.GatewaySpec{
				Port:              legacySpec.Gateway.Port,
				SecurePort:        legacySpec.Gateway.SecurePort,
				Instances:         legacySpec.Gateway.Instances,
