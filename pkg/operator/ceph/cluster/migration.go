...
func (c *ClusterController) watchLegacyClusters(namespace string, stopCh chan struct{}, resourceHandlerFuncs cache.ResourceEventHandlerFuncs) {
	...
	// watch for cluster.ceph.rook.io/v1alpha1 events if the CRD exists
	if _, err := c.context.RookClientset.CephV1alpha1().Clusters(namespace).List(metav1.ListOptions{}); err != nil {
		logger.Infof("skipping watching for legacy ceph cluster events (legacy cluster CRD probably doesn't exist): %+v", err)
	} else {
		logger.Infof("start watching legacy ceph clusters in all namespaces")
		watcherLegacy := opkit.NewWatcher(ClusterResourceCephLegacy, namespace, resourceHandlerFuncs, c.context.RookClientset.CephV1alpha1().RESTClient())
		go watcherLegacy.Watch(&cephv1alpha1.Cluster{}, stopCh)
	}
}
...
func getClusterObject(obj interface{}) (cluster *cephv1beta1.Cluster, migrationNeeded bool, err error) {
	...
	clusterCephLegacy, ok := obj.(*cephv1alpha1.Cluster)
	if ok {
		return convertCephLegacyCluster(clusterCephLegacy.DeepCopy()), true, nil
	}
...
func (c *ClusterController) migrateClusterObject(clusterToMigrate *cephv1beta1.Cluster, legacyObj interface{}) error {
	...
	} else if _, ok := legacyObj.(*cephv1alpha1.Cluster); ok {
		logger.Infof("deleting legacy ceph cluster %s in namespace %s", clusterToMigrate.Name, clusterToMigrate.Namespace)
		return c.context.RookClientset.CephV1alpha1().Clusters(clusterToMigrate.Namespace).Delete(
			clusterToMigrate.Name, &metav1.DeleteOptions{PropagationPolicy: &deletePropagation})
	}
...
// ************************************************************************************************
// Ceph legacy conversion functions (ceph.rook.io/v1alpha1)
// ************************************************************************************************

// converts a legacy cluster.ceph.rook.io/v1alpha1 object to the current cluster.ceph.rook.io/v1beta1 object.
// Traverses through the entire object to convert all specs/fields.
func convertCephLegacyCluster(legacyCluster *cephv1alpha1.Cluster) *cephv1beta1.Cluster {
	if legacyCluster == nil {
		return nil
	}
	...
	// default to `3` mons during upgrade when legacy mon count is `0`
	if legacySpec.Mon.Count <= 0 {
		legacySpec.Mon.Count = mon.DefaultMonCount
	}
	...
	cluster := &cephv1beta1.Cluster{
		...
		Spec: cephv1beta1.ClusterSpec{
			...
			Network:         legacySpec.Network,
			Resources:       legacySpec.Resources,
			ServiceAccount:  legacySpec.ServiceAccount,
			DataDirHostPath: legacySpec.DataDirHostPath,
			Mon: cephv1beta1.MonSpec{
			...
			Dashboard: cephv1beta1.DashboardSpec{
				Enabled: legacySpec.Dashboard.Enabled,
			},
		},
	}
