...
var PoolResourceCephLegacy = opkit.CustomResource{
	Name:    customResourceName,
	Plural:  customResourceNamePlural,
...
func (c *PoolController) watchLegacyPools(namespace string, stopCh chan struct{}, resourceHandlerFuncs cache.ResourceEventHandlerFuncs) {
	...
	// watch for pool.ceph.rook.io/v1alpha1 events if the CRD exists
	if _, err := c.context.RookClientset.CephV1alpha1().Pools(namespace).List(metav1.ListOptions{}); err != nil {
		logger.Infof("skipping watching for legacy ceph pool events (legacy pool CRD probably doesn't exist): %+v", err)
	} else {
		logger.Infof("start watching legacy ceph pools in all namespaces")
		watcherLegacy := opkit.NewWatcher(PoolResourceCephLegacy, namespace, resourceHandlerFuncs, c.context.RookClientset.CephV1alpha1().RESTClient())
		go watcherLegacy.Watch(&cephv1alpha1.Pool{}, stopCh)
	}
}
...
func getPoolObject(obj interface{}) (pool *cephv1beta1.Pool, migrationNeeded bool, err error) {
	...
	poolCephLegacy, ok := obj.(*cephv1alpha1.Pool)
	if ok {
		return convertCephLegacyPool(poolCephLegacy.DeepCopy()), true, nil
	}
...
func (c *PoolController) migratePoolObject(poolToMigrate *cephv1beta1.Pool, legacyObj interface{}) error {
	...
	} else if _, ok := legacyObj.(*cephv1alpha1.Pool); ok {
		logger.Infof("deleting legacy ceph pool %s in namespace %s", poolToMigrate.Name, poolToMigrate.Namespace)
		return c.context.RookClientset.CephV1alpha1().Pools(poolToMigrate.Namespace).Delete(
			poolToMigrate.Name, &metav1.DeleteOptions{PropagationPolicy: &deletePropagation})
	}
...
func ConvertCephLegacyPoolSpec(legacySpec cephv1alpha1.PoolSpec) cephv1beta1.PoolSpec {
	return cephv1beta1.PoolSpec{
		FailureDomain: legacySpec.FailureDomain,
		CrushRoot:     legacySpec.CrushRoot,
		Replicated: cephv1beta1.ReplicatedSpec{
...
func convertCephLegacyPool(legacyPool *cephv1alpha1.Pool) *cephv1beta1.Pool {
	if legacyPool == nil {
		return nil
	}
	...
	pool := &cephv1beta1.Pool{
		...
		Spec: ConvertCephLegacyPoolSpec(legacyPool.Spec),
	}
