...
var FilesystemResourceCephLegacy = opkit.CustomResource{
	Name:    customResourceName,
	Plural:  customResourceNamePlural,
...
func (c *FilesystemController) watchLegacyFilesystems(namespace string, stopCh chan struct{}, resourceHandlerFuncs cache.ResourceEventHandlerFuncs) {
	...
	// watch for ceph.filesystem.rook.io/v1alpha1 events if the CRD exists
	if _, err := c.context.RookClientset.CephV1alpha1().Filesystems(namespace).List(metav1.ListOptions{}); err != nil {
		logger.Infof("skipping watching for legacy ceph filesystem events (legacy filesystem CRD probably doesn't exist): %+v", err)
	} else {
		logger.Infof("start watching legacy ceph filesystems in all namespaces")
		watcherLegacy := opkit.NewWatcher(FilesystemResourceCephLegacy, namespace, resourceHandlerFuncs, c.context.RookClientset.CephV1alpha1().RESTClient())
...
func getFilesystemObject(obj interface{}) (filesystem *cephv1beta1.Filesystem, migrationNeeded bool, err error) {
	...
	filesystemCephLegacy, ok := obj.(*cephv1alpha1.Filesystem)
	if ok {
		return convertCephLegacyFilesystem(filesystemCephLegacy.DeepCopy()), true, nil
	}
...
func (c *FilesystemController) migrateFilesystemObject(filesystemToMigrate *cephv1beta1.Filesystem, legacyObj interface{}) error {
	...
	} else if _, ok := legacyObj.(*cephv1alpha1.Filesystem); ok {
		logger.Infof("deleting legacy ceph filesystem %s in namespace %s", filesystemToMigrate.Name, filesystemToMigrate.Namespace)
		return c.context.RookClientset.CephV1alpha1().Filesystems(filesystemToMigrate.Namespace).Delete(
			filesystemToMigrate.Name, &metav1.DeleteOptions{PropagationPolicy: &deletePropagation})
	}
...
func convertCephLegacyFilesystem(legacyFilesystem *cephv1alpha1.Filesystem) *cephv1beta1.Filesystem {
	if legacyFilesystem == nil {
		return nil
	}
	...
	dataPools := make([]cephv1beta1.PoolSpec, len(legacySpec.DataPools))
	for i, dp := range legacySpec.DataPools {
		dataPools[i] = pool.ConvertCephLegacyPoolSpec(dp)
	}
